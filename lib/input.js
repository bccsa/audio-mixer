...
class Input extends Writable {
  constructor(args) {
    ...
    this._peak10th = new peakBuffer(this._peak100th.outputSampleRate, 1 / 10);
    this._peak3 = new peakBuffer(this._peak10th.outputSampleRate, 3);

    this._peak1000th.onPeakUpdate(peak => {
      this._peak100th.setLevel(peak);
    });
    this._peak100th.onPeakUpdate(peak => {
      this._peak10th.setLevel(peak);
    });
    this._peak10th.onPeakUpdate(peak => {
      this._level = peak;
      this._events.emit('level', this._level);
      this._peak3.setLevel(peak);
...
class peakBuffer {
  constructor(sampleRate, peakLength) {
    // Store samples for 0.001 seconds
