	...
	if (samples) {
		...
		this.inputs.forEach(function (input) {
			...
			for (var i = 0; i < samples * this.channels; i++) {
				this.writeSample.call(mixedBuffer, this.readSample.call(mixedBuffer, i * this.sampleByteLength) + Math.round(this.readSample.call(inputBuffer, i * this.sampleByteLength) * input.gain / this.inputs.length), i * this.sampleByteLength);
			}
		}.bind(this));
