...
const { dampener } = require('./piControl.js');
...
class Mixer {
  ...
  _read() {
    ...
    if (this.inputs) {
      ...
      if (this.chunkSize < minSamples) {
        minSamples = this.chunkSize;
      }
      ...
      if (samples) {
        ...
        for (let j = 0; j < inputs.length; j++) {
          ...
          for (let i = 0; i < samples * this.channels; i++) {
            ...
            let outputSample = this.readSample.call(mixedBuffer, i * this.sampleByteLength) + inputSample * this.volume;
